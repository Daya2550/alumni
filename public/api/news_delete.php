<?php
require_once __DIR__ . '/../../includes/database.php';
require_once __DIR__ . '/../../includes/auth.php';
require_once __DIR__ . '/../../includes/file_cleanup.php';

header('Content-Type: application/json');

if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    http_response_code(405);
    echo json_encode(['success' => false, 'message' => 'Method not allowed']);
    exit;
}

if (!Auth::isLoggedIn()) {
    http_response_code(401);
    echo json_encode(['success' => false, 'message' => 'Authentication required']);
    exit;
}

$user = Auth::getCurrentUser();
$input = json_decode(file_get_contents('php://input'), true);

$post_id = intval($input['post_id'] ?? 0);
$csrf = $input['csrf_token'] ?? '';

if ($post_id <= 0) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'Post ID required']);
    exit;
}

if (!Auth::verifyCSRFToken($csrf)) {
    http_response_code(403);
    echo json_encode(['success' => false, 'message' => 'Invalid CSRF token']);
    exit;
}

try {
    $post = db()->fetchOne("SELECT id, author_id, media_urls FROM news_posts WHERE id = ?", [$post_id]);
    if (!$post) {
        http_response_code(404);
        echo json_encode(['success' => false, 'message' => 'Post not found']);
        exit;
    }

    $isPrivileged = Auth::hasAnyRole(['staff','admin']);
    $isAuthor = ($post['author_id'] === $user['id']);
    if (!($isPrivileged || $isAuthor)) {
        http_response_code(403);
        echo json_encode(['success' => false, 'message' => 'You can only delete your own news']);
        exit;
    }

    // Clean up associated files using centralized system
    $files_deleted = FileCleanup::cleanupNewsFiles($post_id);

    db()->execute("DELETE FROM news_posts WHERE id = ?", [$post_id]);
    
    error_log("News post {$post_id} deleted. Cleaned up {$files_deleted} files.");

    echo json_encode(['success' => true]);
} catch (Exception $e) {
    error_log('News delete error: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Internal server error']);
}
?>










