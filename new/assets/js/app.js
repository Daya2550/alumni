(() => {
    ...
    async function loadColumns(){
		...
		if (!tbl) return;
		try {
			const data = await fetchJSON('api/columns.php?table='+encodeURIComponent(tbl));
    ...
    async function runQuery(){
		...
		try {
			const data = await fetchJSON('api/data.php', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body)});
            if (isRaw) {
                renderKPIs([]);
                showTable(data.rows);
